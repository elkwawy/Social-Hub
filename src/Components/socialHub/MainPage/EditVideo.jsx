...
const EditVideoModal = memo(({videoDetails, updateVideo, loading}) => {
    ...
    useEffect(() => { 
        if (videoDetails)   { 
            setInputs(videoDetails);
            setPrevInputs(videoDetails);
        }
        if (videoDetails.thumbnailURL)
            setimgURL(videoDetails.thumbnailURL);
    }, [videoDetails])
    ...
    const didInputsChange = () => { 
        return JSON.stringify(inputs)!== JSON.stringify(prevInputs);
    }
    ...
    const handleEditVideo = () => { 
        if(isValidInputs()) { 
            if (didInputsChange()) {  
                ...
                showToast("error", "You changed nothing");
            }
        }
    }
    ...
    return (
        <div className='grid md:grid-cols-2 gap-8'>
            ...
            <div className='flex sm:max-w-full flex-col items-start gap-4'>
                ...
                ? <Img onClick={focusOnThumbnailInput} src={inputs.thumbnailURL} className='max-w-full h-48 rounded-md'loader={
